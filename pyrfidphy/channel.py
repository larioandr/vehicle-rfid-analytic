...
def _less_abs_1(value):
    if np.abs(abs(value) - 1.) < 1e-15:
        return 1
    else:
        return value
...
def two_ray_path_loss_3d(*, time, ground_reflection, wavelen,
    ...
    dot_ = np.dot(d0_vector_tx_n, tx_dir_theta)
    tx_azimuth_0 = np.arccos(dot_) if np.abs(dot_) < 1 else 0

    rx_azimuth_0 = np.arccos(_less_abs_1(np.dot(d0_vector_rx_n, rx_dir_theta)))
    tx_azimuth_1 = np.arccos(_less_abs_1(np.dot(d1_vector_tx_n, tx_dir_theta)))
    rx_azimuth_1 = np.arccos(_less_abs_1(np.dot(d1_vector_rx_n, rx_dir_theta)))

    tx_tilt_0 = np.arccos(_less_abs_1(np.dot(d0_vector_tx_n, tx_dir_phi)))
    rx_tilt_0 = np.arccos(_less_abs_1(np.dot(d0_vector_rx_n, rx_dir_phi)))
    tx_tilt_1 = np.arccos(_less_abs_1(np.dot(d1_vector_tx_n, tx_dir_phi)))
    rx_tilt_1 = np.arccos(_less_abs_1(np.dot(d1_vector_rx_n, rx_dir_phi)))
